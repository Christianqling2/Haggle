<template>
  <view class="search-list">
    ...
    <view class="search-top">
      <view class="search-box" @tap="toSearchPage(1)">
        <view class="icon">
          <image src="/static/images/search.png" />
        </view>
        <div v-if="showKeyword" class="key-tag">{{ showKeyword }}<span class="close" @tap.stop="toSearchPage(0)">x</span></div>
        <div v-if="!showKeyword" class="text">输入关键字搜索</div>
      </view>
      <view v-if="state!==3" class="list-style" @tap="changeListStyle">
        <image v-if="isLineProds" src="/static/images/list-row.png" />
        <image v-else src="/static/images/list-line.png" />
      </view>
    </view>
    <!-- 商品头部店铺 -->
    <view v-if="state!==3 && shopInfo" class="head-shop-info">
    ...
    <!-- 筛选栏 -->
    <view class="sortbar">
      <view class="item" :class="{active:state===0}" @tap="switchSearchList(0)">默认</view>
      <view class="item" :class="{active:state===1, 'arrow-up':pageQuery.sort===2, 'arrow-down':pageQuery.sort===3}" @tap="switchSearchList(1)">销量</view>
      <view class="item" :class="{active:state===2, 'arrow-up':pageQuery.sort===4, 'arrow-down':pageQuery.sort===5}" @tap="switchSearchList(2)">价格</view>
    </view>
    <!-- 产品列表 -->
    <view v-if="state!==3 && prodList.length > 0" :class="[isLineProds ? 'line-prods' : 'prods']">
    ...
    <!-- 无商品显示 -->
    <view v-if="state!==3 && prodList.length===0" class="empty">
      <view class="img">
    ...
    <!-- 店铺列表 -->
    <view v-if="state===3 && shopData.total > 0" class="shop-list">
      <block v-for="(item, index) in shopData.list" :key="index">
        <view class="item">
          <view class="shop-info">
            ...
            <view class="text">
              ...
              <view class="focus-box">
                <view v-if="item.type===1" class="self">自营</view>
              </view>
            </view>
            <view class="btn" @tap="toShopPage(item.shopId)">进店</view>
          </view>
    ...
    <!-- 无店铺 -->
    <view v-if="state===3 && shopData.total===0" class="empty">
      <view class="img">
        <image src="/static/empty-img/not-found.png" />
      </view>
      <view class="text">没有找到对应店铺，看看别的吧</view>
    </view>
    <!-- 加载全部 -->
...
export default {
   ...
   */
  onLoad(options) {
    this.pageQuery.categoryId = options.categoryId
    this.showKeyword = options.keyword
    this.pageQuery.keyword = this.pageQuery.categoryId ? '' : this.showKeyword
    this.getSearchList()
  },
  ...
  onReachBottom() {
    if (this.state !==3) {
      ...
      // 店铺下拉触底
      if (this.pageNum < this.shopData.pages) {
  ...
  methods: {
    ...
    getSearchList() {
      ...
      const params = {
        ...
        callBack: res => {
          ...
          this.shopInfo = res.list[0].shopInfo
          let list = []
    ...
    getShopList() {
      ...
      const params = {
        ...
        data: {
          ...
          pageSize: 10,
          shopName: this.pageQuery.keyword
        },
        callBack: res => {
          let list = []
          if (this.pageNum === 1) {
    ...
    switchSearchList(state) {
      ...
      if (state === 1) {
        sort = this.state === state ? this.pageQuery.sort === 2 ? 3 : 2 : 2
      } else if (state === 2) {
        sort = this.state === state ? this.pageQuery.sort === 4 ? 5 : 4 : 4
      }
      this.state = state
      this.pageQuery.sort = sort
     ...
     */
    getSearchContent(e) {
      this.pageQuery.keyword = e.detail.value
    },
     ...
     */
    toSearchPage(type) {
      const url = type === 0 ? '/pages/search-page/search-page' : '/pages/search-page/search-page?keyword=' + this.showKeyword
      uni.redirectTo({ url })
    },
